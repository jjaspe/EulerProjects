    ...
    var addPositive = e.addPositive = function (x, y) {
        forcePositiveString(x);
        forcePositiveString(y);
    ...
    module.exports.add = e.add = function (x, y) {
        forceString(x);
        forceString(y);
        ...
        if (isPositive(x) && isPositive(y)) {
            ...
            // |a| >= |b|
            var absResult = e.subPositive(abs(x), abs(y));
            if (isPositive(x)) {
                // Example: 5 + -3
                return absResult;
            } else {
    ...
    module.exports.sub = e.sub = function (x, y) {
        forceString(x);
        forceString(y);
        return e.add(x, negate(y));
    }

    //------------------- Multiplication

    var mulDigit = e.timesDigit = e.mulDigit = function (strint, digit) {
    ...
    var mulPositive = e.mulPositive = function (lhs, rhs) {
        ...
        for(var i=0; i<digitCount; i++) {
            var singleRow = e.mulDigit(lhs, Number(getDigit(rhs, i)));
            singleRow = shiftLeft(singleRow, i);
            result = e.addPositive(result, singleRow);
        }
        return result;
    }

    module.exports.mul = e.mul = function (lhs, rhs) {
        forceString(lhs);
        forceString(rhs);

        var absResult = e.mulPositive(abs(lhs), abs(rhs));
    ...
    module.exports.eq = e.eq = function (lhs, rhs) {
